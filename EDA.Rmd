...
ggplot(grouped, aes(x = x, y = total_Verkaufsmenge)) +
  ...
  labs(
    title = "Verkaufsmenge über die Zeit",
    x = "Datum",
    y = "Verkaufsmenge"
  ) +
...
# Proteinfladen
grouped <- df %>%
  filter(Artikelnummer == 107) %>%
  mutate(Wochentag = weekdays(ymd(as.character(Datum)))) %>%
  filter(Wochentag != "Samstag") %>%
  group_by(Datum) %>%
  summarise(total_Verkaufsmenge = sum(Verkaufsmenge))

grouped$x <- 1:length(grouped$total_Verkaufsmenge)

spline_model <- lm(total_Verkaufsmenge ~ ns(x, df = 40), data = grouped[0:(-30),])
...
forecasted_values <- data.frame(
  y = predict(spline_model, data.frame(x=tail(grouped$x,30))),
  x = tail(grouped$x, 30)
)

#cols <- c("1" = "black", "2" = "red", "3" = "yellow")

pred_df <- data.frame(x = forecasted_values$x, y = forecasted_values$y)
